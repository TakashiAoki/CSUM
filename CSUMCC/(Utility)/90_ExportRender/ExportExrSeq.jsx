...
// **** FUNCTION ******************************************************************************************************************
//		レンダリング設定・出力モジュールを設定
		function RQItemSetUp()
{
	for ( i = 0; i < curItemList.length; i++ )
	{
		curItemList[i].applyTemplate( curRS );//レンダリング設定
		curItemList[i].outputModule(1).applyTemplate( "QT Animation" );//出力モジュールを設定
		
		var curItemSaveName = curItemList[i].comp.name;
		
		//コンポジション名からファイル名禁止文字を削除
		var str = curItemSaveName;
		var result = "";
		...
		var curItemSaveName = result;
		
		//保存ディレクトリ作成
		var saveDirectory = new Folder( app.project.file.parent.parent.fsName + "/" + "preRender" );
		if ( saveDirectory.exists == false ) saveDirectory.create();
		
		//シーケンス保存フォルダ指定
		var saveFolder = new Folder( saveDirectory.fsName + "/" + curItemSaveName );
		
		//既に保存先フォルダがあった場合の処理
		for ( x = 2; saveFolder.exists == true; x++ )
		{
			if ( x == 2 )
			{ var saveFolder = new Folder(saveFolder.fsName + "_v" + x); }
			else
			{ var saveFolder = new Folder(saveFolder.fsName.slice( 0 , saveFolder.fsName.lastIndexOf("v")+1 ) + x); }
		}
		if ( saveFolder.exists == false ) saveFolder.create();

		//保存パス設定
		var saveFile = new File( saveFolder.fsName + "/" + curItemSaveName + ".mov" );
		
		//出力モジュールを後から変更することで保存ファイル拡張子を自動変更
		curItemList[i].outputModule(1).applyTemplate( curOM );//出力モジュールを設定		
		curItemList[i].outputModule(1).file = saveFile;
		curItemList[i].outputModule(1).postRenderAction = PostRenderAction.IMPORT;
	}
}
// **** FUNCTION ******************************************************************************************************************
//		スクリプトファイルの実行
